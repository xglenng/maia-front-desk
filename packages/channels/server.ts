...
export async function processSocialInbound(event: SocialInboundEvent, origin: string) {
  ...
  if (!client) {
    const profile = process.env.META_MESSAGING_MODE === "mock" ? { id: event.externalUserId, name: event.profileName || `Test ${event.externalUserId}`, username: event.username || event.externalUserId } : await getSocialProfile(event.externalUserId, accessToken);
